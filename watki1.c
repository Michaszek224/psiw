...
int main(int argc, char *argv[]) {
    if (argc < 3) {
        printf("Użyj: %s <MAX_PASSENGERS> <WAGON_CAPACITY>\n", argv[0]);
        return 1;
    }

    MAX_PASSENGERS = atoi(argv[1]);
    WAGON_CAPACITY = atoi(argv[2]);

    pthread_t pasażerowie[MAX_PASSENGERS];
    ...
    // Start the wagon thread after creating passenger threads
    pthread_t wagonik;
